...
class BsScanThruStream(BsInst):
    def __init__(self, operands=None, ret=0):
        super().__init__(BsInstType.SCANTHRUSTREAM, operands, ret)
        self.n_operand = 2
    ...
    def lower_to_cuda(self, var_map, indent: str = "", var_define_map: dict = None):
        ...
        operation = "bs_scan_thru_stream"
        code = f"{indent}{operation}({operand1}, {operand2}, carry_stream, add_stream, {ret}, n_unit_basic);"
        if self.comment != "":
            code += f" // {self.comment}"
        return code
