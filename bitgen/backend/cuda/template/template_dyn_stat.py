...
def init_args(kernel, regex_func_num, bitstream: torch.Tensor):
    ...
    print(
        f"dyn_stats shape: {dyn_stats_shape}, Mem: {cal_tensor_mem_mb(dyn_stats_shape, torch.uint32)} MB"
    )
    print(
        "Total Mem: ",
        cal_tensor_mem_mb(bitstream_shape, torch.uint32)
        + cal_tensor_mem_mb(result_shape, torch.uint32)
        + cal_tensor_mem_mb(dyn_stats_shape, torch.uint32)
    ...
    dyn_stats = dyn_stats.contiguous()
    ...
    for input_split_id in range(n_split_input):
        ...
        dyn_stats_ptr = [int(dyn_stats.data_ptr())]
        ...
        d_dyn_stats = np.array(dyn_stats_ptr, dtype=np.uint64)
    ...
    return cuda_args, result, tmp_streams, dyn_stats
